...
// FlatIndexDirname is where flat-mode repo manifests are stored.
const FlatIndexDirname = ".hfd-flat-index"
...
// FlatIndexPath returns the flat-mode index manifest path for a repo.
func FlatIndexPath(baseDir, repo string) string {
	name := strings.ReplaceAll(repo, "/", "--") + ".yaml"
	return filepath.Join(baseDir, FlatIndexDirname, name)
}

// WriteFlatIndex writes a manifest index for flat-mode downloads.
func (m *DownloadManifest) WriteFlatIndex(baseDir string) (string, error) {
	manifestPath := FlatIndexPath(baseDir, m.Repo)
	manifestDir := filepath.Dir(manifestPath)

	if err := os.MkdirAll(manifestDir, 0o755); err != nil {
		return "", fmt.Errorf("create flat index dir: %w", err)
	}

	data, err := yaml.Marshal(m)
	if err != nil {
		return "", fmt.Errorf("marshal flat index manifest: %w", err)
	}

	header := fmt.Sprintf("# HuggingFace %s: %s\n", m.Type, m.Repo)
	header += fmt.Sprintf("# Downloaded: %s\n", m.CompletedAt.Format(time.RFC3339))
	header += "# Flat mode index generated by hfdownloader\n\n"
	content := header + string(data)

	if err := os.WriteFile(manifestPath, []byte(content), 0o644); err != nil {
		return "", fmt.Errorf("write flat index manifest: %w", err)
	}
